...
describe('StringIsRegExMatch', () => {
  ...
  it('should match regular expression with (5) numbers exactly', () => {
    const target = '12345';
    const expression: RegExp = /^[+ 0-9]{5}$/;
    rule = new StringIsRegExMatch('NumbersRegularExpression', 'The target value must be numbers only.', target, expression, isDisplayable);
    result = rule.render();
